...
void Mapa::gen_current(Level &lvl){
    lvl.generate_level(*it);
}

void Mapa::gen_next(Level &lvl){
    if (it == seed_list.begin()){
        srand(*it);
        seed_list.push_front(rand()%10000);
        it = seed_list.begin();
    }
    else {
        it --;
